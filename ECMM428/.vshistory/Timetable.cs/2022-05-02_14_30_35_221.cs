...
namespace ECMM428
{
    ...
    public class Timetable
    {
        ...
        public double STUDENT_SOFT_CONSTRAINT_3_PENALTY = 0;
        public double STUDENT_SOFT_CONSTRAINT_4_PENALTY = 0;

        //Constants for Socha tests
        /*public int HARD_CONSTRAINT_PENALTY = 10000;
        public int MODULE_SOFT_CONSTRAINT_1_PENALTY = 1;
        public double MODULE_SOFT_CONSTRAINT_2_PENALTY = 0;
        public int COURSE_SOFT_CONSTRAINT_1_PENALTY = 0;
        public double STUDENT_SOFT_CONSTRAINT_1_PENALTY = 0;
        public double STUDENT_SOFT_CONSTRAINT_2_PENALTY = 1;
        public int STUDENT_SOFT_CONSTRAINT_2_CONSECUTIVE_HOURS_ALLOWED = 2;
        ...
        public void GenericInitialization(int seed)
        {
            ...
            for (int i = 0; i < unassignedLectures.Count; i++)
            {
                ...
                for (int j = 0; j < timeslots.Count; j++)
                {
                    ...
                    //Get random venue
                    List<Venue> availableVenues = GetAvailableVenuesAtTime(lecture, time);
        ...
        {
            double fitness = 0;
            if (COURSE_SOFT_CONSTRAINT_1_PENALTY > 0) fitness += CourseSoftConstraint1(course);
        ...
        {
            double fitness = 0;
            if (HARD_CONSTRAINT_PENALTY > 0) fitness += ModuleHardConstraint1(module);
            if (HARD_CONSTRAINT_PENALTY > 0) fitness += ModuleHardConstraint2(module);
            if (MODULE_SOFT_CONSTRAINT_1_PENALTY > 0) fitness += ModuleSoftConstraint1(module);
            if (MODULE_SOFT_CONSTRAINT_2_PENALTY > 0) fitness += ModuleSoftConstraint2(module);
        ...
        {
            double fitness = 0;
            if (STUDENT_SOFT_CONSTRAINT_1_PENALTY > 0) fitness += StudentSoftConstraint1(student);
            if (STUDENT_SOFT_CONSTRAINT_2_PENALTY > 0) fitness += StudentSoftConstraint2(student);
            if (STUDENT_SOFT_CONSTRAINT_3_PENALTY > 0) fitness += StudentSoftConstraint3(student);
            if (STUDENT_SOFT_CONSTRAINT_4_PENALTY > 0) fitness += StudentSoftConstraint4(student);
        ...
        {
            double fitness = 0;
            if (HARD_CONSTRAINT_PENALTY > 0) fitness += LecturerHardConstraint1(lecturer);
        ...
        {
            int noTimeslotsInDay = GetNoTimeslotsInDay();
            List<Lecture> lectures = student.GetTimetabledLectures();
            if (lectures.Count == 0) return 0;
            else if (lectures.Count == 1) return STUDENT_SOFT_CONSTRAINT_4_PENALTY;
            int violations = 0;
            int previousTime = lectures[0].GetTime();
            bool exactlyOneLectureCountedOnCurrentDay = true;

            for(int i = 1; i < lectures.Count; i++)
            {
                int currentTime = lectures[i].GetTime();
                if ((currentTime + 1) % noTimeslotsInDay != (previousTime + 1) % noTimeslotsInDay)
                {
                    if (exactlyOneLectureCountedOnCurrentDay)
                    {
                        violations++;
                    }
                    exactlyOneLectureCountedOnCurrentDay = true;
                } else
                {
                    exactlyOneLectureCountedOnCurrentDay = false;
                }
            }
            double fitness = violations * STUDENT_SOFT_CONSTRAINT_4_PENALTY;
