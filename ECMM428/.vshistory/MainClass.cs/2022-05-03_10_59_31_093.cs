...
namespace ECMM428
{
    public class MainClass
    {
        static void Main(string[] args)
        {
            ...
            int SEED = 0;
            bool ALL_LECTURES_LAST_ONE_HOUR = true;

            //Initialize timetable
            Timetable timetable = MainClass.Initialize(NO_MODULES_PER_STUDENT, NO_COURSES, NO_MODULES_PER_COURSE, 
                NO_LECTURERS_PER_COURSE, NO_ROOMS,  NO_TIMESLOTS_PER_DAY, NO_STUDENTS, HOURS_OF_LECTURES_PER_MODULE, 
                MIN_VENUE_CAPACITY, MAX_VENUE_CAPACITY, SEED, ALL_LECTURES_LAST_ONE_HOUR);


            //timetable = HeuristicAlgorithm.Execute(timetable);
            TestGeneticAlgorithmCrossoverProbability(timetable, 1, 0, 1000, 0, 0.5, 0.05, 100);
            //TestAlgorithmOnline("Genetic", timetable, 0, 0.55, 1, 1000, 0, 0, 1, 10);
            //TestAlgorithmAsynchronous("Differential Evolution", timetable, 0, 0.55, 1, 1000, 0, 2, 2, 10);
            //TestRemoteInitializationAlgorithm("Differential Evolution", "Online", timetable, 0, 0.55, 1, 1000, 0, 0.3, 0.06, 10);
        }
        ...
        public static Timetable Initialize(int noModulesPerStudent, int noCourses, int noModulesPerCourse, int noLecturersPerCourse, int noRooms, int noTimeslotsPerDay, int noStudents, int hoursOfLecturesPerModule, int minVenueCapacity, int maxVenueCapacity, bool allLecturesLastOneHour)
        {
            return Initialize(noModulesPerStudent, noCourses, noModulesPerCourse, noLecturersPerCourse, noRooms, noTimeslotsPerDay, noStudents, hoursOfLecturesPerModule, minVenueCapacity, maxVenueCapacity, new Random().Next(), allLecturesLastOneHour);
        }
            ...
            for (int courseId = 0; courseId < noCourses; courseId++)
            {
                ...
                for (int modNum = 0; modNum < noModulesPerCourse; modNum++)
                {
                    ...
                    while (lectureHoursRemaining > 0)
                    {
                        ...
                        {
                            double lectureDurationDecider = rnd.NextDouble();
                            int lectureDuration;
        ...
        public static void TestRemoteInitializationAlgorithm(string algorithm, string remoteType, Timetable initialTimetable, int seed, double crossoverProbability, double fitnessMargin, int noInMarginToStopAfter, double initialValue, double finalValue, double step, int noRunsPerStep)
        {
            int noSteps = (int)Math.Floor((double)(finalValue - initialValue) / step) + 1;
            double[,] results = new double[noSteps, 5];
            var stopwatch = new System.Diagnostics.Stopwatch();
            ...
            for (int i = 0; i < noSteps; i++)
            {
                ...
                if (remoteType == "Online")
                {
                    currentStepTimetable = OnlineInitializationAlgorithm.Execute(currentStepTimetable, currentValue);
                }
                else if (remoteType == "Asynchronous")
                {
                    currentStepTimetable = AsynchronousInitializationAlgorithm.Execute(currentStepTimetable, currentValue);
                } else
                {
                    throw new ArgumentException("Argument 'remoteType' has an incorrect name.");
                }
                Timetable[] timetableArray = new Timetable[noRunsPerStep];
