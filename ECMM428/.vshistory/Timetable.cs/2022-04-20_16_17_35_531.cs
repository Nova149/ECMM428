...
namespace ECMM428
{
    ...
    public class Module
    {
        ...
        {
            int originalTime = lecture.GetTime();
            Venue originalVenue = lecture.GetVenue();
            lecture.SetVenue(venue);
            lecture.SetTime(time);

            if (venue != null)
            {
                venue.AddLecture(lecture);
                if (originalVenue == null && originalTime > -1) noOnlineLectures--;
                else if (originalVenue == null && originalTime == -1) noAsynchronousLectures--;
            }
            else if (time > -1)
            {
                noOnlineLectures++;
                if (originalVenue == null && originalTime == -1) noAsynchronousLectures--;
            }
            else if (time == -1)
            {
                noAsynchronousLectures++;
                if (originalTime > -1) noAsynchronousLectures--;
            }
