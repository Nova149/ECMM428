...
namespace ECMM428
{
    ...
    public class DifferentialEvolutionAlgorithm : NatureInspiredAlgorithm
    {
        public static Timetable Execute(Timetable timetable, double fitnessMargin, int noToExpandSearchAfter, int noInMarginToStopAfter, int maxOnlineLectures, int maxAsynchronousLectures, int resets)
        {
            ...
            List<Lecture> timetable1DifferentLectures = new List<Lecture>();
            List<Lecture> timetable2DifferentLectures = new List<Lecture>();
            ...
            while (true)
            {
                ...
                {
                    foreach(LectureTransform transform in timetable1NewArray.GetArray())
                    {
                        if (!timetable1DifferentLectures.Contains(transform.GetLecture())) timetable1DifferentLectures.Add(transform.GetLecture());
                    }
                }
                ...
                {
                    foreach (LectureTransform transform in timetable2NewArray.GetArray())
                    {
                        if (!timetable2DifferentLectures.Contains(transform.GetLecture())) timetable2DifferentLectures.Add(transform.GetLecture());
                    }
                }
                ...
                //and remove the worst solution. If the two timetables in the population are both timetable1s or timetable2s, set the one to be removed to be equal to
                //the other
