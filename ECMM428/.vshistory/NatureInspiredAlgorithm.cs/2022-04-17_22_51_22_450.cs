    ...
    public class NatureInspiredAlgorithm
    {
        ...
        public static void Crossover(Timetable timetable1, Timetable timetable2)
        {
            ...
            for (int moduleIndex = 0; moduleIndex < modules1.Count; moduleIndex++)
            {
                ...
                List<Course> lectures1 = new List<Course>(module1.GetLectures());
                List<Course> lectures2 = new List<Course>(module2.GetLectures());
                lectures1.Shuffle();
                lectures2.Shuffle();
                bool lecture1Found = false;
                bool lecture2Found = false;

                Course lecture1 = null;
                Venue lecture1Venue = null;
                int lecture1Time = -1;

                Course lecture2 = null;
                ...
                for (int lectureIndex = 0; lectureIndex < lectures1.Count; lectureIndex++)
                {
                    ...
                    if (lecture1Venue == null || lecture2Venue == null)
                    {
                        ...
                        Course onlineLecture1;
                        Course onlineLecture2;
                        if (lecture1Venue == null)
                        {
                            onlineLecture1 = lecture1;
                            List<Course> time2Lectures = new List<Course>(timetable2.GetOnlineLecturesFromModule(timetable2.GetModule(module1.GetNumber())));
                            if (time2Lectures.Count > 1) onlineLecture2 = time2Lectures.GetRandom();
                            else continue;
                        } else
                        {
                            onlineLecture2 = lecture2;
                            List<Course> time1Lectures = new List<Course>(timetable1.GetOnlineLecturesFromModule(timetable1.GetModule(module1.GetNumber())));
        ...
        public static void Mutation(Timetable timetable)
        {
            ...
            for (int time1Index = 0; time1Index < timeslots1.Count; time1Index++)
            {
                for (int time2Index = 0; time2Index < timeslots2.Count; time2Index++)
                {
                    ...
                    List<Course> time1Lectures = new List<Course>(timetable.GetLecturesStartAtTime(lecture1Time));
                    ...
                    for (int lecture1Index = 0; lecture1Index < time1Lectures.Count; lecture1Index++)
                    {
                        ...
                        Course lecture1 = time1Lectures[lecture1Index];
                        ...
                        for (int courseIndex = 0; courseIndex < courses.Count; courseIndex++)
                        {
                            ...
                            if (lecture1Venue == null)
                            {
                                List<Course> time2Lectures = new List<Course>(timetable.GetOnlineLecturesStartAtTime(lecture2Time));
                            ...
                            else
                            {
                                ...
                                for (int venueIndex = 0; venueIndex < venues.Count; venueIndex++)
                                {
                                    ...
                                    Course lecture2 = timetable.GetLecture(lecture2Course, lecture2Venue, lecture2Time);
        ...
        {
            int maxDifference = 0;
            List<Course> lecturesWithHighestDifference = new List<Course>();
            ...
            foreach (Module m in modules)
            {
                ...
                {
                    List<Course> inPersonLectures = m.GetInPersonLectures();
                    if (inPersonLectures.Count != 0)
                    {
                        Course l = inPersonLectures.GetRandom();
            ...
            if (maxDifference > 0)
            {
                Course lecture = lecturesWithHighestDifference.GetRandom();
        ...
        public static bool LectureToAsynchronousMutation(Timetable timetable, int maxNoAsynchronousLectures)
        {
            ...
            foreach (Module m in modules)
            {
                if (m.GetNoAsynchronousLectures() < maxNoAsynchronousLectures)
                {
                    Course l;
                    List<Course> inPersonLectures = m.GetInPersonLectures();
                    List<Course> onlineLectures = m.GetOnlineLectures();
                    if (inPersonLectures.Count > 0)
                    {
