            ...
            for (int moduleIndex = 0; moduleIndex < modules.Count; moduleIndex++)
            {
                ...
                for (int lectureIndex = 0; lectureIndex < lectures1.Count; lectureIndex++)
                {
                    ...
                    else
                    {
                        ...
                        List<Venue> availableVenues1;
                        List<Venue> availableVenues2;
                        //Get venues for new timeslots
                        if (!lecture2Found)
                        {
                            availableVenues1 = timetable1.GetAvailableVenuesAtTime(lecture2, lecture2Time);
                        }
                        if (!lecture1Found)
                        {
                            availableVenues2 = timetable2.GetAvailableVenuesAtTime(lecture1, lecture1Time);
                        }

                        //Get random venue for each lecture
                        Venue venue1 = null;
                        Venue venue2 = null;
                        if (availableVenues1.Count > 0)
                        {
                            venue1 = availableVenues1.GetRandom();
                            lecture2Found = true;
                        }
                        if (availableVenues2.Count > 0)
                        {
