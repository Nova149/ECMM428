...
namespace ECMM428
{
    public class MainClass
    {
        static void Main(string[] args)
        {
            ...
            Timetable timetable = MainClass.Initialize(NO_MODULES_PER_STUDENT, NO_COURSES, NO_MODULES_PER_COURSE, NO_LECTURERS_PER_COURSE, NO_ROOMS, NO_TIMESLOTS_PER_DAY, NO_STUDENTS, MIN_VENUE_CAPACITY, MAX_VENUE_CAPACITY);
            ...
            //TestGeneticAlgorithmExpand(timetable, 0.125, 50, 1000, 500, 1000, 20, 10);
            //TestGeneticAlgorithmCrossoverProbability(timetable, 50, 1000, 0, 1, 0.05, 10);
            //TestGeneticAlgorithmCrossoverProbability(timetable, 1, 1000, 0, 0.21, 0.01, 5);
            TestAlgorithmOnline("Differential Evolution", timetable, 0.125, 1, 1000, 0, 2, 1, 10);
            //TestAlgorithmAsynchronous("Differential Evolution", timetable, 0.125, 1, 1000, 0, 2, 1, 5);
        ...
        {
            return Initialize(noModulesPerStudent, noCourses, noModulesPerCourse, noLecturersPerCourse, noRooms, noTimeslotsPerDay, noStudents, minVenueCapacity, maxVenueCapacity, false);
        }
        public static Timetable Initialize(int noModulesPerStudent, int noCourses, int noModulesPerCourse, int noLecturersPerCourse, int noRooms, int noTimeslotsPerDay, int noStudents, int minVenueCapacity, int maxVenueCapacity, bool allLecturesLastOneHour)
        {
            //Constants
        ...
        public static void TestAlgorithmExpand(string algorithm, Timetable initialTimetable, double crossoverProbability, double fitnessMargin, int noInMarginToStopAfter, int initialValue, int finalValue, int step, int noRunsPerStep)
        {
            ...
            for (int i = 0; i < noSteps; i++)
            {
                ...
                for (int j = 0; j < noRunsPerStep; j++)
                {
                    ...
                    stopwatch.Stop();
                    resultsInStep[j] = timetable.FitnessFunction();
        ...
        public static void TestGeneticAlgorithmCrossoverProbability(Timetable initialTimetable, double fitnessMargin, int noInMarginToStopAfter, double initialValue, double finalValue, double step, int noRunsPerStep)
        {
            int noSteps = (int)Math.Floor((double)(finalValue - initialValue) / step) + 1;
            double[,] results = new double[noSteps, 3];
            var stopwatch = new System.Diagnostics.Stopwatch();
            ...
            for (int i = 0; i < noSteps; i++)
            {
                ...
                for (int j = 0; j < noRunsPerStep; j++)
                {
                    ...
                    stopwatch.Stop();
                    resultsFitnessInStep[j] = timetable.FitnessFunction();
                    resultsStopwatchInStep[j] = stopwatch.ElapsedMilliseconds;
                }
                results[i, 0] = currentValue;
        ...
        public static void TestAlgorithmOnline(string algorithm, Timetable initialTimetable, double fitnessMargin, int noInMarginToStopAfter, int initialValue, int finalValue, int step, int noRunsPerStep)
        {
            TestAlgorithmOnline(algorithm, initialTimetable, 0, fitnessMargin, noInMarginToStopAfter, initialValue, finalValue, step, noRunsPerStep);
        }
        public static void TestAlgorithmOnline(string algorithm, Timetable initialTimetable, double crossoverProbability, double fitnessMargin, int noInMarginToStopAfter, int initialValue, int finalValue, int step, int noRunsPerStep)
        {
            int noSteps = (int)Math.Floor((double)(finalValue - initialValue) / step) + 1;
            ...
            for (int i = 0; i < noSteps; i++)
            {
                ...
                double[] noOnlineLecturesInStep = new double[noRunsPerStep];
                for (int j = 0; j < noRunsPerStep; j++)
                {
                    System.Diagnostics.Debug.WriteLine("j = " + j + " / " + (noRunsPerStep - 1));
                    ...
                    noOnlineLecturesInStep[j] = totalOnlineCount;

                 }
                results[i, 0] = currentValue;
                results[i, 1] = Util.GetAverage(resultsFitnessInStep);
                results[i, 2] = Util.GetAverage(resultsStopwatchInStep);
                results[i, 3] = Util.GetAverage(fractionTotalLecturesOnlineInStep);
                results[i, 4] = Util.GetAverage(noOnlineLecturesInStep);
            }

            ExportToExcel(results);
        }
        public static void TestAlgorithmAsynchronous(string algorithm, Timetable initialTimetable, double crossoverProbability, double fitnessMargin, int noInMarginToStopAfter, int initialValue, int finalValue, int step, int noRunsPerStep)
        {
            int noSteps = (int)Math.Floor((double)(finalValue - initialValue) / step) + 1;
            double[,] results = new double[noSteps, 5];
            var stopwatch = new System.Diagnostics.Stopwatch();
            ...
            for (int i = 0; i < noSteps; i++)
            {
                ...
                double[] noAsynchronousLecturesInStep = new double[noRunsPerStep];
                for (int j = 0; j < noRunsPerStep; j++)
                {
                    System.Diagnostics.Debug.WriteLine("j = " + j + " / " + (noRunsPerStep - 1));
                    ...
                    noAsynchronousLecturesInStep[j] = totalAsynchronousCount;
                }
                results[i, 0] = currentValue;
                results[i, 1] = Util.GetAverage(resultsFitnessInStep);
                results[i, 2] = Util.GetAverage(resultsStopwatchInStep);
                results[i, 3] = Util.GetAverage(fractionTotalLecturesAsynchronousInStep);
                results[i, 4] = Util.GetAverage(noAsynchronousLecturesInStep);
            }
