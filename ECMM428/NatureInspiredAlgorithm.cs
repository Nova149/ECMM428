...
namespace ECMM428
{
    ...
    public class NatureInspiredAlgorithm
    {
        ...
        public static LectureTransform[] Crossover(Timetable timetable1, Timetable timetable2, bool revertIfWorse)
        {
            ...
            modules1.Shuffle();
            modules2.Shuffle();
            LectureTransform transform1 = null;
            LectureTransform transform2 = null;
            ...
            for (int moduleIndex = 0; moduleIndex < modules1.Count; moduleIndex++)
            {
                ...
                for (int lectureIndex = 0; lectureIndex < lectures1.Count; lectureIndex++)
                {
                    ...
                    for (int lectureIndex2 = 0; lectureIndex2 < lectures2.Count; lectureIndex2++)
                    {
                        if (!lecture2Found)
                        {
                            lecture2 = lectures2[lectureIndex2];
                            lecture2Venue = lecture2.GetVenue();
                            lecture2Time = lecture2.GetTime();
                        }
                    }

                    //If a lecture is online
                    if (lecture1Venue == null || lecture2Venue == null)
                    {
                        ...
                        bool improvedByNewLecture1 = timetable1InitialFitness >= timetable1.GetFitness();
                        bool improvedByNewLecture2 = timetable2InitialFitness >= timetable2.GetFitness();
                        if (revertIfWorse)
                        {
                            //If either of the timetables have a better fitness value, keep it. Otherwise, reverse the operation
                            if (!improvedByNewLecture1)
                            {
                                timetable1.MakeLectureOnline(onlineLecture1, onlineLecture1Time);
                                timetable1.RecalculateFitnessValues(onlineLecture1);
                            }

                            if (!improvedByNewLecture2)
                            {
                        ...
                        {
                            //Generate transforms
                            if (lecture1 != null) transform1 = new LectureTransform(onlineLecture1, lecture1Venue, null, onlineLecture1Time, onlineLecture2Time);
                            if (lecture2 != null) transform2 = new LectureTransform(onlineLecture2, lecture2Venue, null, onlineLecture2Time, onlineLecture1Time);
                        }
                        return new LectureTransform[2] { transform1, transform2 };
                    }
                    else
                    {
                        //Unassign lectures
                        ...
                        {
                            //Generate transforms
                            if (lecture1 != null) transform1 = new LectureTransform(lecture1, lecture1Venue, newVenue1, lecture1Time, lecture2Time);
                            if (lecture2 != null) transform2 = new LectureTransform(lecture2, lecture2Venue, newVenue2, lecture2Time, lecture1Time);
                        }
                        return new LectureTransform[2] { transform1, transform2 };
                    }
                }
        ...
        /*public static LectureTransform[] Crossover(Timetable timetable1, Timetable timetable2, int times)
        {
            for (int i = 0; i < times; i++) Crossover(timetable1, timetable2, true);
        }*/

        public static LectureTransform[] Crossover(Timetable timetable1, Timetable timetable2, List<int> timetable1LecturesChanged, List<int> timetable2LecturesChanged)
        {
            return Crossover(timetable1, timetable2, false);
        }

        public static LectureTransform[] Crossover(Timetable timetable1, Timetable timetable2)
        {
            return Crossover(timetable1, timetable2, false);
        }

        ///Swap 2 random lectures in different timeslots from the same timetable
        public static LectureTransform[] Mutation(Timetable timetable, bool revertIfWorse)
        {
            double timetableInitialFitness = timetable.GetFitness();
            //Initialize timeslots
            int noTimeslots = timetable.GetNoTimeslots();
            if (noTimeslots <= 1) return null;
                    ...
                    for (int lecture1Index = 0; lecture1Index < time1Lectures.Count; lecture1Index++)
                    {
                        ...
                        for (int courseIndex = 0; courseIndex < courses.Count; courseIndex++)
                        {
                            ...
                            if (lecture1Venue == null)
                            {
                                ...
                                timetable.MakeLectureOnline(lecture1, lecture2Time);
                                ...
                                if (time2Lectures.Count != 0)
                                {
                                    ...
                                    if (rnd.Next(0, 2) == 0)
                                    {
                                        ...
                                        timetable.MakeLectureOnline(lecture2, lecture1Time);
                                        timetable.RecalculateFitnessValues(lecture2);
                                    }
                                }

                                bool improvedByNewLectures = timetableInitialFitness >= timetable.GetFitness();
                                if (revertIfWorse)
                                {
                                    //If the timetable has a better fitness value, keep it. Otherwise, reverse the operation
                                    if (!improvedByNewLectures)
                                    {
                                        timetable.UnassignLecture(lecture1);
                                        timetable.MakeLectureOnline(lecture1, lecture1Time);
                                        timetable.RecalculateFitnessValues(lecture1);

                                        if (lecture2Swapped)
                                        {
                                            timetable.UnassignLecture(lecture2);
                                            timetable.MakeLectureOnline(lecture2, lecture2Time);
                                            timetable.RecalculateFitnessValues(lecture2);
                                        }
                                    }
                                } 
                                ...
                                {
                                    //Generate transforms
                                    if (lecture1 != null) transform1 = new LectureTransform(lecture1, null, null, lecture1Time, lecture2Time);
                                    if (lecture2Swapped) transform2 = new LectureTransform(lecture2, null, null, lecture2Time, lecture1Time);
                                }
                                return new LectureTransform[2] { transform1, transform2 };
                            }
                            ...
                            else
                            {
                                ...
                                for (int venueIndex = 0; venueIndex < venues.Count; venueIndex++)
                                {
                                    ...
                                    //If no venues are available, reassign lecture1 and lecture2 and break
                                    ...
                                    //Perform swap
                                    Venue newVenue1 = venuesForLecture1.GetRandom();
                                    timetable.SetSlot(lecture1, newVenue1, lecture2Time);

                                    Venue newVenue2 = null;
                                    if (lecture2 != null)
                                    {
                                        newVenue2 = venuesForLecture2.GetRandom();
                                        timetable.SetSlot(lecture2, newVenue2, lecture1Time);
                                        timetable.RecalculateFitnessValues(lecture2);
                                    }
                                    timetable.RecalculateFitnessValues(lecture1);

                                    bool improvedByNewLectures = timetableInitialFitness >= timetable.GetFitness();
                                    if (revertIfWorse && !improvedByNewLectures)
                                    {
                                        //If the timetable has a better fitness value, keep it. Otherwise, reverse the operation
                                        timetable.UnassignLecture(lecture1);
                                        timetable.SetSlot(lecture1, lecture1Venue, lecture1Time);
        ...
        /*public static LectureTransform[] Mutation(Timetable timetable, int times)
        {
            for (int i = 0; i < times; i++) Mutation(timetable, null, false);
        }*/
        public static LectureTransform[] Mutation(Timetable timetable)
        {
            return Mutation(timetable, false);
        }
