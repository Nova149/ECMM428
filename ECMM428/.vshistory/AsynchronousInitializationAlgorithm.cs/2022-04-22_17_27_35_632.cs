    ...
    public class AsynchronousInitializationAlgorithm
    {
        public static Timetable Execute(Timetable timetable, double fracToMakeAsynchronous)
        {
            ...
            while (!allAssign)
            {
                ...
                //Add in size order, with smaller groups having lower indices
                if (g.Count == 0 || gm.Count <= g[0].Count) g.Add(gm);
                else
                {
                    for (int i = 1; i < g.Count; i++)
                    {
                        if (gm.Count > g[i].Count) g.Insert(i, gm);
                        break;
                    }
                    g.Add(gm);
                }
            ...
            int noAsynchronous = 0;
            int maxAsynchronous = (int)Math.Ceiling(timetable.GetLectures().Count * fracToMakeAsynchronous);
            if (maxAsynchronous > 0)
            {
                for (int i = 0; i < g.Count; i++)
                {
                    ...
                    {
                        List<Lecture> gmLectures = gm[j].GetLectures();
                        for (int k = 0; k < gmLectures.Count; k++)
                        {
                            gmLectures[k].MakeAsynchronous();
                            noAsynchronous++;
                            if (noAsynchronous > maxAsynchronous) return timetable;
                        }
                    }
                }
            }
