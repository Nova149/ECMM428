...
namespace ECMM428
{
    ...
    public class DifferentialEvolutionAlgorithm : NatureInspiredAlgorithm
    {
        public static Timetable Execute(Timetable timetable, double fitnessMargin, int noToExpandSearchAfter, int noInMarginToStopAfter, int maxOnlineLectures, int maxAsynchronousLectures, int resets)
        {
            ...
            //Initialize LectureTransformArrays to keep track of how to transform from timetable1_1 to timetable1_2 (and vice versa)
            //and likewise for timetable2_1 and timetable2_2
            LectureTransformArray timetable1Transform = new LectureTransformArray();
            LectureTransformArray timetable2Transform = new LectureTransformArray();
            ...
            while (true)
            {
                ...
                double parent1NewFitness = parent1.GetFitness();
                double parent2NewFitness = parent2.GetFitness();
                ...
                if (parent1Fitness > parent1NewFitness)
                {
                    timetable1NewArray.PerformReverseTransform(parent1, 1);
                }
                else
                {
                    timetable1Transform.JoinArrays(timetable1NewArray, 2);
                }
                if (parent2Fitness > parent2NewFitness)
                {
                    timetable1NewArray.PerformReverseTransform(parent2, 1);
                }
                else
                {
                    timetable2Transform.JoinArrays(timetable2NewArray, 2);
                }
