...
namespace ECMM428
{
    public class MainClass
    {
        static void Main(string[] args)
        {
            ...
            //TestAlgorithmOnline("Differential Evolution", timetable, 0, 0.55, 1, 1000, 0, 2, 1, 10);
            //TestAlgorithmAsynchronous("Differential Evolution", timetable, 0, 0.55, 1, 1000, 0, 2, 1, 100);
            TestAsynchronousInitializationAlgorithm("Differential Evolution", timetable, 0, 0.55, 1, 1000, 0, 0.2, 0.05, 10);
        ...
        public static void TestAsynchronousInitializationAlgorithm(string algorithm, Timetable initialTimetable, int seed, double crossoverProbability, double fitnessMargin, int noInMarginToStopAfter, int initialValue, int finalValue, int step, int noRunsPerStep)
        {
            int noSteps = (int)Math.Floor((double)(finalValue - initialValue) / step) + 1;
            double[,] results = new double[noSteps, 7];
            var stopwatch = new System.Diagnostics.Stopwatch();
            ...
            for (int i = 0; i < noSteps; i++)
            {
                ...
                initialTimetable = AsynchronousInitializationAlgorithm.Execute(initialTimetable, currentValue);
                Timetable[] timetableArray = new Timetable[noRunsPerStep];
