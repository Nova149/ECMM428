...
namespace ECMM428
{
    ...
    public class DifferentialEvolutionAlgorithm : NatureInspiredAlgorithm
    {
        public static Timetable Execute(Timetable timetable, double fitnessMargin, int noToExpandSearchAfter, int noInMarginToStopAfter, int maxOnlineLectures, int maxAsynchronousLectures, int resets)
        {
            ...
            while (true)
            {
                //Get two random timetables from the population vector
                Random rng = new Random();
                int index1 = rng.Next(0, population.Count);
                int index2 = index1;
                while (index2 == index1)
                {
                    index2 = rng.Next(0, population.Count);
                }
                Timetable parent1 = population[index1];
                Timetable parent2 = population[index2];
                ...
                double child1Fitness = child1.FitnessFunction();
                double child2Fitness = child2.FitnessFunction();
