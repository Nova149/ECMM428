...
namespace ECMM428
{
    public class Student
    {
        ...
        private List<Course> assignedLectures;
        double fitness = 0;
        ...
        public Student(int studentNumber, int yearOfStudy, List<Module> enrolledModules)
        {
            ...
            this.assignedLectures = new List<Course>();
        }
        ...
        public List<Course> GetLectures() { return assignedLectures; }
        public List<Course> GetTimetabledLectures()
        {
        ...
        public void AssignLecture(Course lecture)
        {
        ...
        public void AssignLectures(List<Course> lectures)
        {
        ...
        public void UnassignLecture(Course lecture)
        {
    ...
    public class Course
    {
        ...
        Venue venue;
        int time;
        public Course(int lectureNumber, int duration, Module module, Venue venue, int time)
        {
    ...
    public class Module
    {
        ...
        private List<Course> unassignedLectures = new List<Course>();
        private int noOnlineLectures = 0;
        private int noAsynchronousLectures = 0;
        private Course course;
        double fitness = 0;

        public Module(int moduleNumber, List<Student> enrolledStudents, Lecturer lecturer, List<Course> lectures, Course course)
        {
            this.moduleNumber = moduleNumber;
            this.enrolledStudents = new List<Student>(enrolledStudents);
            this.lecturer = lecturer;
            this.lectures = new List<Course>(lectures);
            unassignedLectures = new List<Course>(lectures);
            this.course = course;
        }

        public Module(int moduleNumber, List<Student> enrolledStudents, Lecturer lecturer, List<Course> lectures, Course course, List<Course> unassignedLectures, double fitness)
        {
            this.moduleNumber = moduleNumber;
        ...
        public Module(int moduleNumber, List<Student> enrolledStudents, Lecturer lecturer, List<Course> lectures, Course course, List<Course> unassignedLectures, int noOnlineLectures, int noAsynchronousLectures, double fitness)
        {
            this.moduleNumber = moduleNumber;
            this.enrolledStudents = enrolledStudents;
            this.lecturer = lecturer;
        ...
        {
            Course lecture = new Course(number, durationOfLecture, this, null, 0);
        ...
        public List<Course> GetLectures() { return lectures; }
        public List<Course> GetTimetabledLectures()
        {
        ...
        public List<Course> GetInPersonLectures() {
            List<Course> inPersonLectures = new List<Course>();
            foreach (Course l in lectures)
            {
                if (l.GetVenue() != null) inPersonLectures.Add(l);
            }
            return inPersonLectures; 
        }
        public List<Course> GetOnlineLectures()
        {
            List<Course> onlineLectures = new List<Course>();
            foreach (Course l in lectures)
            {
                if (l.GetVenue() == null && l.GetTime() > -1) onlineLectures.Add(l);
            }
            return onlineLectures;
        }
        public void SetLectures(List<Course> lectures) { this.lectures = lectures; }
        public Course GetLecture(Venue venue, int time)
        {
            foreach (Course lecture in GetLectures())
            {
                if (lecture.GetVenue() == venue && lecture.GetTime() == time) return lecture;
            }
            return null;
        }
        public List<Course> GetUnassignedLectures() { return unassignedLectures; }
        public void SetUnassignedLectures(List<Course> unassignedLectures) { this.unassignedLectures = unassignedLectures; }
        public List<Course> GetAssignedLectures()
        {
            List<Course> assignedLectures = new List<Course>();
            foreach (Course lecture in lectures)
            {
        ...
        {
            List<Course> lectures = GetLectures();
        ...
        public double GetFitness() { return fitness; }
        public void SetFitness(double fitness) { this.fitness = fitness; }
        public void AssignLecture(Course lecture, Venue venue, int time)
        {
        ...
        {
            Course lecture = null;
            foreach (Course l in unassignedLectures)
            {
        ...
        public void AddUnassignedLecture(Course lecture)
        {
            unassignedLectures.Add(lecture);
        }
        ...
        public void RecalculateFitnessValues()
        {
    ...
    public class Course
    {
        ...
        public List<Course> GetLectures()
        {
            ...
            {
                foreach (Course l in m.GetLectures())
                {
                    lectures.Add(l);
                }
            }
    ...
    public class Venue
    {
        ...
        private Dictionary<int, Course> lectures;

        public Venue(int venueNumber, int venueCapacity, int noTimeslots)
        {
            this.venueNumber = venueNumber;
            this.venueCapacity = venueCapacity;
            lectures = new Dictionary<int, Course>();
            for (int i = 0; i < noTimeslots; i++)
            {
                lectures.Add(i, null);
            }
        }
        public Venue(int venueNumber, int venueCapacity, Dictionary<int, Course> lectures)
        {
            this.venueNumber = venueNumber;
            this.venueCapacity = venueCapacity;
            this.lectures = lectures;
        ...
        public Dictionary<int, Course> GetLectures() { return lectures; }
        ...
        public Course GetLectureAt(int time)
        {
    ...
    public class Timetable
    {
        ...
        public int HARD_CONSTRAINT_PENALTY = 10000;
        public int MODULE_SOFT_CONSTRAINT_1_PENALTY = 1;
        public int COURSE_SOFT_CONSTRAINT_1_PENALTY = 1;
        public double STUDENT_SOFT_CONSTRAINT_1_PENALTY = 0.1;
        public double STUDENT_SOFT_CONSTRAINT_2_PENALTY = 0.1;
        public int STUDENT_SOFT_CONSTRAINT_2_CONSECUTIVE_HOURS_ALLOWED = 2;
        public double MODULE_SOFT_CONSTRAINT_2_PENALTY = 3;
        ...
        public Timetable(Timetable timetable)
        {
            ...
            foreach (Course c in timetable.GetCourses())
            {
                ...
                foreach (Module m in c.GetModules())
                {
                    ...
                    //Copy lectures
                    List<Course> newLectures = new List<Course>();
                    List<Course> newUnassignedLectures = new List<Course>();
                    foreach (Course l in m.GetLectures())
                    {
                        ...
                        //Create newLecture
                        Course newLecture = new Course(l.GetNumber(), l.GetDuration(), newModule, lectureVenue, l.GetTime());
        ...
        public List<Course> GetUnassignedLectures()
        {
            ...
            foreach (Course c in courses)
            {
                foreach (Module m in c.GetModules())
                {
                    foreach (Course l in m.GetUnassignedLectures())
                    {
                        lectures.Add(l);
                    }
                }
        ...
        public List<List<Course>> GetLecturesByCourse()
        {
            List<List<Course>> lectures = new List<List<Course>>();
            foreach (Course c in courses)
            {
                List<Course> lecturesInCourse = new List<Course>();
                foreach (Module m in c.GetModules())
                {
                    foreach (Course l in m.GetLectures())
                    {
        ...
        public List<List<Course>> GetLecturesByModule()
        {
            List<List<Course>> lectures = new List<List<Course>>();
            foreach (Module m in GetModules())
            {
                List<Course> lecturesInModule = new List<Course>();
                foreach (Course l in m.GetLectures())
                {
        ...
        public List<Course> GetLectures(Course course)
        {
            List<Course> lectures = new List<Course>();
        ...
        public List<Course> GetLectures(Module module)
        {
            List<Course> lectures = new List<Course>();
            foreach (Course l in module.GetLectures())
            {
                lectures.Add(l);
            }
            return lectures;
        }
        public List<Course> GetLecturesStartAtTime(int time)
        {
            List<Course> lectures = new List<Course>();
            foreach (Module m in GetModules())
            {
        ...
        public List<Course> GetOnlineLecturesStartAtTime(int time)
        {
            List<Course> lectures = new List<Course>();
            foreach (Module m in GetModules())
            {
        ...
        public List<Course> GetOnlineLecturesFromModule(Module module)
        {
            List<Course> lectures = new List<Course>();
            foreach (Course l in module.GetLectures())
            {
        ...
        public List<Course> GetLecturesStartAtTime(int day, int time)
        {
        ...
        public List<Venue> GetAvailableVenuesAtTime(Course lecture, int time)
        {
        ...
        public List<Venue> GetAvailableVenuesAtTime(Course lecture, int day, int time)
        {
        ...
        public void ChangeFitness(double value) { fitness += value; }
        ...
        public void SetSlot(Course lecture, Venue venue, int time)
        {
        ...
        public void SetSlot(Course lecture, Venue venue, int day, int time)
        {
            SetSlot(lecture, venue, day * timeslotsPerDay + time);
        }
        public void MakeLectureOnline(Course lecture, int time)
        {
            if (lecture.GetVenue() != null)
            {
                int courseNo = lecture.GetCourse().GetNumber();
                int venueNo = lecture.GetVenue().GetNumber();
                int lectureTime = lecture.GetTime();
                for (int i = 0; i < lecture.GetDuration(); i++)
                {
                    timetable[courseNo, venueNo, lectureTime + i] = null;
                }
            }
            lecture.MakeOnline(time);
        }
        public void MakeLectureOnline(Course lecture, int day, int time)
        {
            MakeLectureOnline(lecture, day * timeslotsPerDay + time);
        }
        public void MakeLectureOnline(Course lecture)
        {
            MakeLectureOnline(lecture, lecture.GetTime());
        }
        public void MakeLectureAsynchronous(Course lecture)
        {
            MakeLectureOnline(lecture, -1);
        }
        public bool LectureCanBeAllocatedAt(Course lecture, Venue venue, int time)
        {
            ...
            Dictionary<int, Course> lectures = venue.GetLectures();
        ...
        public bool LectureCanBeAllocatedAt(Course lecture, Venue venue, int day, int time)
        {
        ...
        {
            List<Course> unassignedLectures = GetUnassignedLectures();
            ...
            {
                Course lecture = unassignedLectures[i];
                ...
                for (int j = 0; j < timeslots.Count; j++)
                {
                    ...
                    if (availableVenues.Count != 0)
                    {
                        ...
                        RecalculateFitnessValues(lecture);
                        break;
        ...
        public void CalculateAllFitnessValues()
        {
            foreach(Course course in courses)
            {
                CourseConstraints(course);
                ...
                foreach (Lecturer lec in course.GetLecturers())
                {
                    LecturerConstraints(lec);
                }
                foreach (Student student in course.GetStudents())
                {
                    StudentConstraints(student);
                }
            }
        }

        public void RecalculateFitnessValues(Course lecture)
        {
            CourseConstraints(lecture.GetCourse());
            ModuleConstraints(lecture.GetModule());
            foreach (Student student in lecture.GetModule().GetStudents())
            {
                StudentConstraints(student);
            }
            LecturerConstraints(lecture.GetLecturer());
        }
        ...
        public void CourseConstraints(Course course)
        {
            double fitness = 0;
            fitness += CourseSoftConstraint1(course);
            double change = course.GetFitness() - fitness;
            course.SetFitness(fitness);
            this.fitness -= change;
        }
        public void ModuleConstraints(Module module)
        {
            double fitness = 0;
            ...
            double change = module.GetFitness() - fitness;
            module.SetFitness(fitness);
            this.fitness -= change;
        }
        public void StudentConstraints(Student student)
        {
            double fitness = 0;
            fitness += StudentSoftConstraint1(student);
            fitness += StudentSoftConstraint2(student);
            double change = student.GetFitness() - fitness;
            student.SetFitness(fitness);
            this.fitness -= change;
        }
        public void LecturerConstraints(Lecturer lecturer)
        {
            double fitness = 0;
            fitness += LecturerHardConstraint1(lecturer);
            double change = lecturer.GetFitness() - fitness;
            lecturer.SetFitness(fitness);
            this.fitness -= change;
        }
        ...
        public double ModuleHardConstraint1(Module module)
        {
            ...
            foreach (Course l in module.GetTimetabledLectures())
            {
        ...
        public double LecturerHardConstraint1(Lecturer lecturer)
        {
            ...
            List<Course> lectures = lecturer.GetTimetabledLectures();
        ...
        {
            List<Course> lectures = module.GetLectures();
            double fitness = 0;
            foreach (Course lecture in lectures)
            {
                if (lecture.GetVenue() != null)
        ...
        public double CourseSoftConstraint1(Course course)
        {
            ...
            List<Course> lectures = course.GetLectures();
            int totalDuration = 0;
        ...
        public double StudentSoftConstraint1(Student student)
        {
            ...
            foreach (Course l in student.GetTimetabledLectures())
            {
                if (l.GetTime() != -1)
                {
        ...
        public double StudentSoftConstraint2(Student student)
        {
            ...
            double fitness = consecutiveHoursPenalty * STUDENT_SOFT_CONSTRAINT_2_PENALTY;
        ...
        public double ActualFitnessFunction()
        {
            if (this == null) return -1;
            ...
            int SOFT_CONSTRAINT_1_PENALTY = 1;
            int SOFT_CONSTRAINT_2_PENALTY = 1;
            double SOFT_CONSTRAINT_3_PENALTY = 0.1;
            double SOFT_CONSTRAINT_4_PENALTY = 0.1;
            int SOFT_CONSTRAINT_4_CONSECUTIVE_HOURS_ALLOWED = 2;
            double SOFT_CONSTRAINT_5_PENALTY = 3;

            //Initialize variables
            double fitness = 0;
            fitness += HardConstraint1(HARD_CONSTRAINT_PENALTY);
            fitness += HardConstraint2(HARD_CONSTRAINT_PENALTY);
            fitness += HardConstraint3(HARD_CONSTRAINT_PENALTY);
            fitness += SoftConstraint1(SOFT_CONSTRAINT_1_PENALTY);
            fitness += SoftConstraint2(SOFT_CONSTRAINT_2_PENALTY);
            fitness += SoftConstraint3(SOFT_CONSTRAINT_3_PENALTY);
            fitness += SoftConstraint4(SOFT_CONSTRAINT_4_PENALTY, SOFT_CONSTRAINT_4_CONSECUTIVE_HOURS_ALLOWED);
            fitness += SoftConstraint5(SOFT_CONSTRAINT_5_PENALTY);

            return fitness;
        }

        ///Hard constraint 1: All lectures of the same module must occur at different times
        public double HardConstraint1(int penalty)
        {
            int fitness = 0;
            ...
            {
                List<int> lectureTimes = new List<int>();
                foreach (Course l in m.GetTimetabledLectures())
                {
                    int lectureTime = l.GetTime();
                    if (lectureTime != -1)
                    {
                        if (!lectureTimes.Contains(lectureTime)) lectureTimes.Add(lectureTime);
                        else fitness += penalty;
                    }
                }
            }
        ...
        ///Hard constraint 2: All lectures should be assigned
        public double HardConstraint2(int penalty)
        {
            int fitness = 0;
            List<Module> modules = GetModules();
            ...
            foreach (Module m in modules)
            {
                fitness += m.GetUnassignedLectures().Count * penalty;
            }
            return fitness;
        }

        ///Hard constraint 3: No lecturer should have more than one simultaneous lecture
        public double HardConstraint3(int penalty)
        {
            ...
            {
                List<Course> lectures = lec.GetTimetabledLectures();
                int lastLectureFinishTime = lectures[0].GetTime() + lectures[0].GetDuration();
                int nextLectureStartTime;
                for (int i = 1; i < lectures.Count; i++)
                {
                    nextLectureStartTime = lectures[i].GetTime();
                    if (nextLectureStartTime < lastLectureFinishTime) fitness += penalty;
                    lastLectureFinishTime = lectures[i].GetTime() + lectures[i].GetDuration();
                }
            }
            return fitness;
        ...
        ///Soft constraint 1: Lectures should be assigned to rooms with sufficient capacity for all the students to attend the lecture
        public double SoftConstraint1(double penalty)
        {
            List<Course> lectures = GetLectures();
            double fitness = 0;
            foreach (Course lecture in lectures)
            {
                ...
                {
                    int difference = lecture.GetModule().GetStudents().Count - lecture.GetVenue().GetCapacity();
                    if (difference > 0) fitness += difference * penalty;
                }
            }
        ...
        ///Soft constraint 2: Lectures of a course should take place over a minimum number of days in the week
        public double SoftConstraint2(double penalty)
        {
            double fitness = 0;
            int noTimeslotsInDay = GetNoTimeslotsInDay();
            List<List<Course>> lecturesByCourse = GetLecturesByCourse();
            for (int i = 0; i < lecturesByCourse.Count; i++)
            {
                List<Course> lecturesInCourse = lecturesByCourse[i];
                int totalDuration = 0;
                List<int> daysWithLectures = new List<int>();
                foreach (Course lecture in lecturesInCourse)
                {
                    if (lecture.GetTime() != -1)
                    {
                        totalDuration += lecture.GetDuration();
        ...
        ///Soft constraint 3: Each student's lectures on the same day should be held as close together as possible
        public double SoftConstraint3(double penalty)
        {
            double fitness = 0;
            int noTimeslotsInDay = GetNoTimeslotsInDay();
            int totalGap = 0;
            ...
            {
                int previousLectureTime = -1;
                foreach (Course l in s.GetTimetabledLectures())
                {
                    if (l.GetTime() != -1)
                    {
                        int currentLectureTime = l.GetTime();
            ...
            fitness += totalGap * penalty;

            return fitness;
        }

        ///Soft constraint 4: Each student should have no more than SOFT_CONSTRAINT_4_CONSECUTIVE_HOURS_ALLOWED hours of lectures in a row on the same day
        public double SoftConstraint4(double penalty, int consecutiveHoursAllowed)
        {
            int noTimeslotsInDay = GetNoTimeslotsInDay();
            int consecutiveHoursPenalty = 0;
            //Iterate over every student
                    ...
                    if (l.GetTime() >= 0)
                    {
                        ...
                        if (Math.Floor((double)(currentLectureTime) / noTimeslotsInDay) == Math.Floor((double)(previousLectureTime) / noTimeslotsInDay))
                        {
                            ...
                            if (difference == 1)
                            {
                                consecutiveHoursInCurrentStreak++;
                                //Express penalty as a multiple of a power of 2, since higher numbers of hours in a row are more serious
                                if (consecutiveHoursInCurrentStreak > consecutiveHoursAllowed) consecutiveHoursPenalty += (int)Math.Pow(2, consecutiveHoursInCurrentStreak - consecutiveHoursAllowed - 1);
                            }
                            else
                            {
                                consecutiveHoursInCurrentStreak = 1;
            ...
            double fitness = consecutiveHoursPenalty * penalty;

            return fitness;
        }

        ///Soft constraint 5: A module should not have multiple lectures on the same day
        public double SoftConstraint5(double penalty)
        {
            int noTimeslotsInDay = GetNoTimeslotsInDay();
            int noDays = GetNoOfDays();
            int additionalLecturesInADay = 0;
            ...
            double fitness = additionalLecturesInADay * penalty;
