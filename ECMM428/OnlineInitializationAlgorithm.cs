...
namespace ECMM428
{
    ...
    public class OnlineInitializationAlgorithm
    {
        public static Timetable Execute(Timetable timetable, double fracToMakeOnline)
        {
            ...
            int noOnline = 0;
            int maxOnline = (int)Math.Ceiling(timetable.GetLectures().Count * fracToMakeOnline);

            foreach(Module m in modulesInOrder)
            {
                foreach (Lecture l in m.GetLectures())
                {
                    if (noOnline == maxOnline) break;

                    timetable.MakeLectureOnline(l);
                    noOnline++;
                }
                if (noOnline == maxOnline) break;
            }
