    ...
    public class NatureInspiredAlgorithm
    {
        ...
        public static void Crossover(Timetable timetable1, Timetable timetable2)
        {
            ...
            for (int moduleIndex = 0; moduleIndex < modules.Count; moduleIndex++)
            {
                ...
                List<Lecture> lectures2 = new List<Lecture>(module.GetLectures());
                lectures1.Shuffle();
                lectures2.Shuffle();
                ...
                Module lecture1Module = null;
                int lecture1Duration = -1;
                Venue lecture1Venue = null;
                Course lecture1Course = null;
                int lecture1Time = -1;
                ...
                for (int lectureIndex = 0; lectureIndex < lectures1.Count; lectureIndex++)
                {
                    ...
                    if (!lecture1Found)
                    {
                        ...
                        lecture1Module = lecture1.GetModule();
                        lecture1Duration = lecture1.GetDuration();
                        lecture1Venue = lecture1.GetVenue();
                        lecture1Course = lecture1.GetCourse();
                    ...
                    if (lecture1Venue == null || lecture2Venue == null)
                    {
                        ...
                        {
                            List<Lecture> time1Lectures = new List<Lecture>(timetable1.GetOnlineLecturesFromModule(timetable1.GetModule(module.GetNumber())));
                            if (time1Lectures.Count > 1) lecture1 = time1Lectures.GetRandom();
                            else continue;
                        }


                        //Move lecture1 to lecture2's time
                        timetable1.MakeLectureOnline(lecture1, lecture2Time);

                        //Move lecture2 to lecture1's time
                        timetable2.MakeLectureOnline(lecture2, lecture1Time);
                        ...
                        {
                            timetable1.MakeLectureOnline(lecture1, lecture1Time);
                            timetable1.RecalculateFitnessValues(lecture1);
                        }
                        if (timetable2InitialFitness > timetable2.GetFitness())
                        {
                            timetable2.MakeLectureOnline(lecture2, lecture2Time);
                            timetable2.RecalculateFitnessValues(lecture2);
                        }
                        return;
                    }
                    else
                    {
                        ...
                        {
                            venue1 = availableVenues1.GetRandom();
                            lecture2Found = true;
                        }
                        if (availableVenues2 != null && availableVenues2.Count > 0)
                        {
                            venue2 = availableVenues2.GetRandom();
                            lecture1Found = true;
                        }
                        ...
                        //Set new timeslots
                        timetable1.SetSlot(lecture1, venue1, lecture2Time);
                        timetable2.SetSlot(lecture2, venue2, lecture1Time);
                        ...
                        if (timetable1.GetFitness() > 10000 || timetable2.GetFitness() > 10000)
                        {
                            int hi = 0;
                        }
                        return;
