...
namespace ECMM428
{
    ...
    public class Lecture
    {
        ...
        public void MakeOnline(int time)
        {
            ...
            module.AssignLecture(this, null, time);
        }
    ...
    public class Module
    {
        ...
        public List<Lecture> GetOnlineLectures()
        {
            List<Lecture> onlineLectures = new List<Lecture>();
            foreach (Lecture l in lectures)
            {
    ...
    public class Timetable
    {
        ...
        private Dictionary<int, Lecture> lectureIndexes = new Dictionary<int, Lecture>();
        double fitness = 0;
        ...
        public double STUDENT_SOFT_CONSTRAINT_3_PENALTY = 1;
        public double STUDENT_SOFT_CONSTRAINT_4_PENALTY = 1;
            ...
            foreach (Course c in timetable.GetCourses())
            {
                ...
                foreach (Module m in c.GetModules())
                {
                    ...
                    foreach (Lecture l in m.GetLectures())
                    {
                        ...
                        lectureIndexes.Add(newLecture.GetNumber(), newLecture);
        ...
        public Timetable(List<Course> courses, List<Venue> venues, int numberOfTimes)
        {
            ...
            foreach (Course course in courses)
            {
                ...
                {
                    lectureIndexes.Add(lecture.GetNumber(), lecture);
                }
            }
        ...
        public List<Lecture> GetAsynchronousLectures()
        {
            List<Lecture> lectures = new List<Lecture>();
            foreach (Course c in courses)
            {
        ...
        public Venue GetVenue(int venueNo)
        {
            return venueIndexes[venueNo];
        }
        ...
        public void SetLecturesFromList(List<Lecture> list)
        {
            foreach (Lecture lecFromList in list)
            {
                Lecture l = lectureIndexes[lecFromList.GetNumber()];
                UnassignLecture(l);
                if (lecFromList.GetVenue() == null)
                {
                    MakeLectureOnline(l, lecFromList.GetTime());
                } else
                {
                    Venue newVenue = venueIndexes[lecFromList.GetVenue().GetNumber()];
                    SetSlot(l, newVenue, lecFromList.GetTime());
                }
                RecalculateFitnessValues(l);
            }
        }
        ...
        public void RecalculateFitnessValues(Lecture lecture)
        {
        ...
        ///All lectures of the same module must occur at different times
        ...
        ///All lectures should be assigned
        ...
        //No lecturer should have more than one simultaneous lecture
        ...
        ///Lectures should be assigned to rooms with sufficient capacity for all the students to attend the lecture
        ...
        ///A module should not have multiple lectures on the same day
        ...
        ///Lectures of a course should take place over a minimum number of days in the week
        ...
        ///Each student's lectures on the same day should be held as close together as possible
        ...
        ///Each student should have no more than STUDENT_SOFT_CONSTRAINT_2_CONSECUTIVE_HOURS_ALLOWED hours of lectures in a row on the same day
        ...
        ///Students should not have a lecture in the last slot of a day
        public double StudentSoftConstraint3(Student student)
        {
            int noTimeslotsInDay = GetNoTimeslotsInDay();
            int lecturesInLastSlot = 0;
            foreach (Lecture l in student.GetTimetabledLectures())
            {
                int time = l.GetTime();
                if (time >= 0 && (time + 1) % noTimeslotsInDay == 0)
                {
                    lecturesInLastSlot++;
                }
            }
            double fitness = lecturesInLastSlot * STUDENT_SOFT_CONSTRAINT_3_PENALTY;

            return fitness;
        }
        ///Students should not have exactly one lecture on a day
        public double StudentSoftConstraint4(Student student)
        {
            int noTimeslotsInDay = GetNoTimeslotsInDay();
            int lecturesInLastSlot = 0;
            foreach (Lecture l in student.GetTimetabledLectures())
        ...
        public double HardConstraint1(int penalty)
        {
            ...
            {
                List<int> lectureTimes = new List<int>();
                foreach (Lecture l in m.GetTimetabledLectures())
                {
                    int lectureTime = l.GetTime();
                    if (lectureTime != -1)
                    {
        ...
        public double HardConstraint3(int penalty)
        {
            ...
            {
                List<Lecture> lectures = lec.GetTimetabledLectures();
                int lastLectureFinishTime = lectures[0].GetTime() + lectures[0].GetDuration();
                int nextLectureStartTime;
                for (int i = 1; i < lectures.Count; i++)
                {
        ...
        public double SoftConstraint2(double penalty)
        {
            double fitness = 0;
            int noTimeslotsInDay = GetNoTimeslotsInDay();
            List<List<Lecture>> lecturesByCourse = GetLecturesByCourse();
            for (int i = 0; i < lecturesByCourse.Count; i++)
            {
                List<Lecture> lecturesInCourse = lecturesByCourse[i];
                int totalDuration = 0;
                List<int> daysWithLectures = new List<int>();
                foreach (Lecture lecture in lecturesInCourse)
                {
                    if (lecture.GetTime() != -1)
                    {
                        totalDuration += lecture.GetDuration();
        ...
        public double SoftConstraint3(double penalty)
        {
            ...
            {
                int previousLectureTime = -1;
                foreach (Lecture l in s.GetTimetabledLectures())
                {
                    if (l.GetTime() != -1)
                    {
                        int currentLectureTime = l.GetTime();
