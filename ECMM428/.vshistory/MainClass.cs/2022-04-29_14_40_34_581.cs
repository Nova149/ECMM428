...
namespace ECMM428
{
    public class MainClass
    {
        static void Main(string[] args)
        {
            ...
            Timetable timetable = MainClass.Initialize(NO_MODULES_PER_STUDENT, NO_COURSES, NO_MODULES_PER_COURSE, NO_LECTURERS_PER_COURSE, NO_ROOMS, NO_TIMESLOTS_PER_DAY, NO_STUDENTS, HOURS_OF_LECTURES_PER_STUDENT, MIN_VENUE_CAPACITY, MAX_VENUE_CAPACITY, SEED, true);
            //timetable = HeuristicAlgorithm.Execute(timetable);
            //timetable = AsynchronousInitializationAlgorithm.Execute(timetable, 0.2);
            timetable = GeneticAlgorithm.Execute(timetable, 0.55, 1, 1000, 0, 2, 2);
            int noLecturesAsynchronous = timetable.GetAsynchronousLectures().Count;
            int breakpoint = 0;
        ...
        public static Timetable Initialize(int noModulesPerStudent, int noCourses, int noModulesPerCourse, int noLecturersPerCourse, int noRooms, int noTimeslotsPerDay, int noStudents, int hoursOfLecturesPerStudent, int minVenueCapacity, int maxVenueCapacity)
        {
            return Initialize(noModulesPerStudent, noCourses, noModulesPerCourse, noLecturersPerCourse, noRooms, noTimeslotsPerDay, noStudents, hoursOfLecturesPerStudent, minVenueCapacity, maxVenueCapacity, new Random().Next(), false);
        }
        public static Timetable Initialize(int noModulesPerStudent, int noCourses, int noModulesPerCourse, int noLecturersPerCourse, int noRooms, int noTimeslotsPerDay, int noStudents, int hoursOfLecturesPerStudent, int minVenueCapacity, int maxVenueCapacity, int seed)
        {
            return Initialize(noModulesPerStudent, noCourses, noModulesPerCourse, noLecturersPerCourse, noRooms, noTimeslotsPerDay, noStudents, hoursOfLecturesPerStudent, minVenueCapacity, maxVenueCapacity, seed, false);
        }
        public static Timetable Initialize(int noModulesPerStudent, int noCourses, int noModulesPerCourse, int noLecturersPerCourse, int noRooms, int noTimeslotsPerDay, int noStudents, int hoursOfLecturesPerStudent, int minVenueCapacity, int maxVenueCapacity, int seed, bool allLecturesLastOneHour)
        {
            //Constants
            ...
            for (int courseId = 0; courseId < noCourses; courseId++)
            {
                ...
                for (int modNum = 0; modNum < noModulesPerCourse; modNum++)
                {
                    ...
                    int lectureHoursRemaining = hoursOfLecturesPerStudent;
        ...
        public static void TestAsynchronousInitializationAlgorithm(string algorithm, Timetable initialTimetable, int seed, double crossoverProbability, double fitnessMargin, int noInMarginToStopAfter, double initialValue, double finalValue, double step, int noRunsPerStep)
        {
            ...
            for (int i = 0; i < noSteps; i++)
            {
                ...
                for (int j = 0; j < noRunsPerStep; j++)
                {
                    ...
                    if (algorithm.Equals("Genetic"))
                    {
                        timetable = GeneticAlgorithm.Execute(timetable, crossoverProbability, fitnessMargin, noInMarginToStopAfter, noInMarginToStopAfter, 0, 0, 2);
                    }
                    else if (algorithm.Equals("Differential Evolution"))
                    {
