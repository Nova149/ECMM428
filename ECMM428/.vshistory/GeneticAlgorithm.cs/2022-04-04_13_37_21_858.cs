...
namespace ECMM428
{
    ...
    public class GeneticAlgorithm : NatureInspiredAlgorithm
    {
        public static Timetable Execute(Timetable timetable, double crossoverProbability, double fitnessMargin, int noToExpandSearchAfter, int noInMarginToStopAfter, int maxOnlineLectures, int maxAsynchronousLectures, int resets)
        {
            ...
            double timetable1BestFitness = timetable1.FitnessFunction();
            double timetable2BestFitness = timetable2.FitnessFunction();
            int noIdentical = 0;
            int currentIndex = 0;
            ...
            while (true)
            {
                ...
                Timetable newTimetable1 = new Timetable(timetable1);
                Timetable newTimetable2 = new Timetable(timetable2);
                ...
                if ((canMoveLectureOnline || canMoveLectureAsynchronous) && rnd.Next(0, 2) == 0)
                {
                    ...
                    {
                        bool lectureMoved1 = LectureToOnlineMutation(newTimetable1, currentMaxOnlineLectures);
                        bool lectureMoved2 = LectureToOnlineMutation(newTimetable2, currentMaxOnlineLectures);
                        if (!lectureMoved1 && !lectureMoved2) canMoveLectureOnline = false;
                    }
                    else if (canMoveLectureAsynchronous)
                    {
                        bool lectureMoved1 = LectureToAsynchronousMutation(newTimetable1, currentMaxAsynchronousLectures);
                        bool lectureMoved2 = LectureToAsynchronousMutation(newTimetable2, currentMaxAsynchronousLectures);
                ...
                else
                {
                    ...
                    {
                        //Perform crossover
                        Crossover(newTimetable1, newTimetable2, noOperations);
                    }
                    else
                    {
                        //Perform mutation
                        Mutation(newTimetable1, noOperations);
                        Mutation(newTimetable2, noOperations);
                    }
                }

                //Compare old and new timetables. Keep the timetable with the better (smaller) fitness value
                double newTimetable1Fitness = newTimetable1.FitnessFunction();
                double newTimetable2Fitness = newTimetable2.FitnessFunction();

                if (newTimetable1Fitness < timetable1BestFitness)
                {
                    timetable1 = newTimetable1;
                    timetable1BestFitness = newTimetable1Fitness;
                }
                if (newTimetable2Fitness < timetable2BestFitness)
                {
                    timetable2 = newTimetable2;
                    timetable2BestFitness = newTimetable2Fitness;
                }

                //Update fitness histories
                fitnessHistoryTimetable1[currentIndex] = timetable1BestFitness;
                fitnessHistoryTimetable2[currentIndex] = timetable2BestFitness;
                ...
                if (timetable1Satisfied || timetable2Satisfied)
                {
                    ...
                    if (currentReset < resets)
                    {
                        ...
                        currentIndex = 0;
                        noIdentical = 0;
                        for (int i = 1; i < fitnessHistoryTimetable1.Length; ++i)
                        {
                            fitnessHistoryTimetable1[i] = -1;
                        }
                        for (int i = 1; i < fitnessHistoryTimetable2.Length; ++i)
                        {
                            fitnessHistoryTimetable2[i] = -1;
                        }
                    }
