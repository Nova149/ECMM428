...
namespace ECMM428
{
    public class MainClass
    {
        static void Main(string[] args)
        {
            ...
            Timetable timetable = MainClass.Initialize(NO_MODULES_PER_STUDENT, NO_COURSES, NO_MODULES_PER_COURSE, NO_LECTURERS_PER_COURSE, NO_ROOMS, NO_TIMESLOTS_PER_DAY, NO_STUDENTS, MIN_VENUE_CAPACITY, MAX_VENUE_CAPACITY, SEED);
            ...
            //TestAlgorithmOnline("Genetic", timetable, 0, 0.125, 1, 1000, 0, 2, 1, 1);
            TestAlgorithmAsynchronous("Genetic", timetable, 0, 0.125, 1, 1000, 0, 2, 1, 5);
        ...
        {
            return Initialize(noModulesPerStudent, noCourses, noModulesPerCourse, noLecturersPerCourse, noRooms, noTimeslotsPerDay, noStudents, minVenueCapacity, maxVenueCapacity, Guid.NewGuid().GetHashCode(), false);
        }
        ...
        public static void TestAlgorithmAsynchronous(string algorithm, Timetable initialTimetable, int seed, double crossoverProbability, double fitnessMargin, int noInMarginToStopAfter, int initialValue, int finalValue, int step, int noRunsPerStep)
        {
            ...
            for (int i = 0; i < noSteps; i++)
            {
                ...
                results[i, 2] = Util.GetAverage(resultsStopwatchInStep);
                results[i, 2] = Util.GetStandardDeviation(resultsStopwatchInStep);
                results[i, 3] = Util.GetAverage(fractionTotalLecturesAsynchronousInStep);
                results[i, 2] = Util.GetStandardDeviation(fractionTotalLecturesAsynchronousInStep);
            }
