...
namespace ECMM428
{
    ...
    public class GeneticAlgorithm : NatureInspiredAlgorithm
    {
        public static Timetable Execute(Timetable timetable, double crossoverProbability, double fitnessMargin, int noToExpandSearchAfter, int noInMarginToStopAfter, int maxOnlineLectures, int maxAsynchronousLectures, int resets)
        {
            ...
            double timetable1Fitness = timetable.GetFitness();
            ...
            while (true)
            {
                ...
                if ((canMoveLectureOnline || canMoveLectureAsynchronous) && rnd.Next(0, 2) == 0)
                {
                    ...
                    {
                        bool lectureMoved1 = LectureToOnlineMutation(timetable, currentMaxOnlineLectures);
                    ...
                    {
                        bool lectureMoved1 = LectureToAsynchronousMutation(timetable, currentMaxAsynchronousLectures);
                ...
                else
                {
                    ...
                    {
                        //Perform crossover
                        Crossover(timetable, timetable2, noOperations);
                        if (timetable.GetFitness() > 10000 || timetable2.GetFitness() > 10000)
                        {
                            int hi = 0;
                        }
                    }
                    ...
                    {
                        //Perform mutation
                        Mutation(timetable, noOperations);
                ...
                double newTimetable1Fitness = timetable.GetFitness();
                ...
                if (timetable1Satisfied || timetable2Satisfied)
                {
                    ...
                    {
                        if (fitnessHistoryTimetable1[currentIndex] < fitnessHistoryTimetable2[currentIndex]) return timetable;
                        else return timetable2;
                    }
                    else if (timetable1Satisfied) return timetable;
                    else return timetable2;
                }
            }
        }
